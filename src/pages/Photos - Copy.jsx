...
import { Camera, ImageIcon, Upload, Plus, Grid3X3, X, Loader2, Shuffle } from 'lucide-react';
...
function Photos() {
  ...
  useEffect(() => {
    ...
    // This avoids React's warning about effects returning non-cleanup values.
    loadImages();
  }, []);
  ...
  const handleImageRemove = async (imageIndex) => {
    ...
    if (!isObjEmpty(imagesObj) && imagesObj[imageIndex]) {
      ...
      const imageId = imageIndex + 1;
      await deleteImage(shortid, imageId);

      // Now, shift the remaining images left
      for (let i = imageIndex; i < 9; i++) {
        if (imagesObj[i + 1]) {
          const fromName = imagesObj[i + 1].split('?')[0]; // remove timestamp part
          const toName = IMAGE_NAMES[i];
          const timestamp = imagesObj[i + 1].split('?t=')[1] || Date.now();

          // Copy the file to new name
          const copyRes = await copyImage(shortid, fromName, toName);
          if (copyRes.success) {
            // Update imagesObj
            imagesObj[i] = `${toName}?t=${timestamp}`;
            // Delete the old file
            const oldImageId = (i + 1) + 1; // i+1 is the index, +1 for 1-based
            await deleteImage(shortid, oldImageId);
          } else {
            console.error('Failed to copy image:', copyRes.msg);
            // Perhaps break or handle error
          }
        } else {
          imagesObj[i] = '';
        }
      }
      imagesObj[9] = ''; // Ensure last is empty
      ...
      if (res.success) {
        ...
        alert('Failed to update images after deletion. Please try again.');
      }
    }
  ...
  const handleReorganizePhotos = async () => {
    ...
    // Update profile
    const res = await updateUserInfo(user?.id, { images: newImagesObj });
    if (res.success) {
      setProfiledata({ ...profiledata, images: newImagesObj });
      // Update local images
      const newImages = Array(10).fill(null);
      for (let i = 0; i < 10; i++) {
        if (newImagesObj[i]) {
          newImages[i] = { uri: `${CDNURL}/${shortid}/${newImagesObj[i]}` };
        }
      }
      setImages(newImages);
    } else {
  ...
  const handleShufflePhotos = async () => {
    ...
    if (nonEmptyIndices.length <= 1) return; // Nothing to shuffle

    // Get the non-empty images with their data
    const photosToShuffle = nonEmptyIndices.map(idx => ({
      index: idx,
      name: imagesObj[idx].split('?')[0],
      timestamp: imagesObj[idx].split('?t=')[1] || Date.now()
    }));

    // Shuffle the array using Fisher-Yates algorithm
    for (let i = photosToShuffle.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [photosToShuffle[i], photosToShuffle[j]] = [photosToShuffle[j], photosToShuffle[i]];
    }

    // Clean up any existing temp files from previous failed attempts
    for (const photo of photosToShuffle) {
      const tempName = `temp_${photo.name}`;
      try {
        await deleteImageByName(shortid, tempName);
      } catch (e) {
        // Ignore errors, file might not exist
    ...
    // Copy from temp to new sequential positions
    const newImagesObj = Array(10).fill('');
    for (let i = 0; i < tempCopies.length; i++) {
      const tempItem = tempCopies[i];
      const toName = IMAGE_NAMES[i];
      const copyRes = await copyImage(shortid, tempItem.tempName, toName);
      if (!copyRes.success) {
        console.error('Failed to copy from temp:', copyRes.msg);
    ...
    if (res.success) {
      ...
      console.error('Failed to update profile:', res.msg);
      alert(`Failed to update shuffled photos: ${res.msg || 'Unknown error'}. Please try again.`);
    }
  };
  ...
  return (
      <div className="container mx-auto px-4 py-8">
        <Card className="w-full shadow-xl border-0 bg-white backdrop-blur-sm">
            ...
            {images.filter(img => img !== null).length > 1 && (
              <div className="flex justify-center mb-8">
                <Button
                  ...
                  variant="outline"
                  size="lg"
                  className="bg-white hover:bg-purple-50 border-2 border-purple-200 hover:border-purple-300 text-purple-600 hover:text-purple-700 font-medium px-8 py-3 rounded-xl shadow-md hover:shadow-lg transition-all duration-300"
                >
